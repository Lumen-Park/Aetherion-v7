    ...
    def run(self, code: str) -> Dict:
        ...
        try:
            cmd = [
                ...
                "--read-only", "--tmpfs", "/tmp:rw,noexec,nosuid,size=50M",
                "--security-opt", "no-new-privileges:true", "--cap-drop", "ALL",
                "-v", f"{temp_path}:/app/script.py:ro", self.image, "python", "-I", "/app/script.py"
            ]
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=self.timeout)
            return {"stdout": result.stdout, "stderr": result.stderr, "returncode": result.returncode, "passed": result.returncode == 0}
