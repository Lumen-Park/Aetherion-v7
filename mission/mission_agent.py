...
class ScoutAgent:
    def __init__(self): self.llm = LLMWrapper()
    def search_github_issues(self, query: str, language: str = "python", limit: int = 10) -> List[Dict]:
        ...
        if resp.status_code == 200:
            return [{"title": i['title'], "body": i['body'], "url": i['html_url'], "repo": i['repository_url'].split('/')[-1], "labels": [l['name'] for l in i['labels']]} for i in resp.json().get('items', [])]
        return []
    def search_web(self, query: str) -> List[str]:
        soup = BeautifulSoup(requests.get(f"https://html.duckduckgo.com/html/?q={query}").text, 'html.parser')
...
class FilterAgent:
    ...
    def filter_issues(self, issues: List[Dict]) -> List[Dict]:
        ...
        for issue in issues:
            ...
            try:
                if json.loads(resp['content']).get('suitable'): filtered.append(issue)
            except: pass
        return filtered
...
class SelectorAgent:
    ...
    def select_best(self, issues: List[Dict]) -> Optional[Dict]:
        ...
        resp = self.llm.generate(prompt)
        try: return issues[json.loads(resp['content']).get('selected_index', 0)]
        except: return issues[0]
...
class GitPayloadBuilder:
    ...
    def build_payload(self, issue: Dict, solution_code: str, explanation: str) -> Dict:
        ...
        payload = {"issue_url": issue['url'], "commit_message": commit_msg, "patch": solution_code, "explanation": explanation}
        os.makedirs("./git_payloads", exist_ok=True)
        with open(os.path.join("./git_payloads", f"payload_{int(time.time())}.json"), 'w') as f:
