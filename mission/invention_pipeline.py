...
class InventionPipeline:
    ...
    def _refine_idea(self, idea: str) -> str:
        return self.llm.generate(f"Refine this invention idea: {idea}")[
            "content"
        ]

    def _develop_theory(self) -> str:
        return self.llm.generate(
            f"Develop a scientific theory: {self.state['refined_idea']}",
            system="You are a theoretical scientist.",
        )["content"]

    def _formulate_hypothesis(self) -> str:
        return self.llm.generate(
            f"Formulate a testable hypothesis: {self.state['theory']}"
        )["content"]

    def _simulate(self) -> str:
        return self.llm.generate(
            f"Mentally simulate this invention and identify failure points: {self.state['refined_idea']}"
        )["content"]

    def _design(self) -> Dict:
        return {
            "spec": self.llm.generate(
                f"Create design specification: {self.state['refined_idea']}"
            )["content"]
        }

    def _college_review(self) -> Dict:
        curator = Curator()
        experts = curator.select_experts(
            self.state["refined_idea"], max_experts=5
        )
        reviews = {}
        for name in experts:
    ...
    def _generate_latex(self) -> str:
        ...
        path = os.path.join(
            "./latex_docs", f"invention_{int(time.time())}.tex"
        )
