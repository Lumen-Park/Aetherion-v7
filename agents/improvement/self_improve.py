...
class CodeAuditAgent:
    ...
    def audit(self, file_path: str) -> AuditReport:
        ...
        return AuditReport(file_path=file_path, issues=issues, confidence=response["confidence"])
...
class SafeApply:
    ...
    def __init__(self, backup_dir: str = "./backups", changelog_dir: str = "./changelog"):
    ...
    def apply(self, proposal: ChangeProposal) -> bool:
        ...
        for file_path in proposal.affected_files:
            backup_path = os.path.join(self.backup_dir, f"{os.path.basename(file_path)}.{int(time.time())}")
        ...
        try:
            subprocess.run(["patch", "-p1", "-i", diff_file], check=True, cwd=".")
    ...
    def _log_change(self, proposal: ChangeProposal):
        ...
        log_file = os.path.join(self.changelog_dir, f"change_{int(time.time())}.json")
