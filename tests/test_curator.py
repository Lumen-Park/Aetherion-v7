    ...
    with patch.object(curator.llm, "generate", return_value=mock_response):
        with patch("agents.governance.curator.AGENT_REGISTRY", mock_registry):
            experts = curator.select_experts(
                "Design a new battery", max_experts=2
            )
...
def test_curator_handles_malformed_response():
    curator = Curator()
    mock_response = {
        "content": "I think you need a physicist and a chemist.",
        "confidence": 0.5,
    }
    with patch.object(curator.llm, "generate", return_value=mock_response):
...
def test_curator_filters_invalid_agent_names():
    curator = Curator()
    mock_response = {
        "content": '["PhysicistAgent", "FakeAgent", "ChemistAgent"]',
        "confidence": 0.9,
    }
    mock_registry = {
        "PhysicistAgent": MagicMock(),
...
def test_keyword_fallback_returns_agents():
    curator = Curator()
    with patch.object(
        curator.llm, "generate", side_effect=Exception("LLM down")
    ):
...
def test_curator_respects_max_experts():
    curator = Curator()
    mock_response = {
        "content": '["PhysicistAgent", "ChemistAgent", "BiologistAgent", "EconomistAgent"]',
        "confidence": 0.9,
    }
    mock_registry = {
        "PhysicistAgent": MagicMock(),
